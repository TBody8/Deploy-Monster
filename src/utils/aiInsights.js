...
class AIInsightsEngine {
  ...
  analyzeTimePreferences(data) {
    // Simulate time-based consumption analysis
    const timeSlots = {
    ...
    // Mock analysis based on consumption patterns
    const totalDrinks = data.reduce((sum, day) => sum + day.drinks.length, 0);
    
    if (totalDrinks > 0) {
      // Simulate realistic time preferences
      timeSlots.morning.count = Math.floor(totalDrinks * 0.4);
      timeSlots.afternoon.count = Math.floor(totalDrinks * 0.45);
      timeSlots.evening.count = totalDrinks - timeSlots.morning.count - timeSlots.afternoon.count;
      
      timeSlots.morning.percentage = ((timeSlots.morning.count / totalDrinks) * 100).toFixed(1);
      timeSlots.afternoon.percentage = ((timeSlots.afternoon.count / totalDrinks) * 100).toFixed(1);
      timeSlots.evening.percentage = ((timeSlots.evening.count / totalDrinks) * 100).toFixed(1);
    }
  ...
  analyzeCaffeineTolerancePattern(data) {
    ...
    if (averageDaily > 400) {
      toleranceLevel = 'High';
      recommendation = 'Consider reducing intake gradually';
    } else if (averageDaily > 200) {
      toleranceLevel = 'Moderate';
      recommendation = 'Good balance, monitor for side effects';
    }
  ...
  analyzeSpendingHabits(data) {
    ...
    const monthlyProjection = (totalCost / data.length) * 30;
  ...
  predictStreakContinuation(data) {
    const recentDays = data.slice(-7); // Last 7 days
    const consistency = recentDays.filter(day => day.drinks.length > 0).length / 7;
    
    let probability = 'Low';
    if (consistency > 0.8) probability = 'High';
    else if (consistency > 0.5) probability = 'Medium';
